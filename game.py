...
class Game:
    ...
    def __perform_border_checking(self):
        ...
        if x <= -self.width / 2:
            self.points_b += 1
            self.__ball_pos = (0, 0)
            self.__ball_delta_x *= choice([-1, 1])
    ...
    def __perform_paddle_hit_checking(self):
        ...
        hit_paddle_a = a_x + self.paddle_width == x and (a_y - self.paddle_height / 2) <= y <= (
                a_y + self.paddle_height / 2)

        b_x, b_y = self.paddle_b_pos
        hit_paddle_b = b_x - self.paddle_width == x and (b_y - self.paddle_height / 2) <= y <= (
                    b_y + self.paddle_height / 2)
